...
from app.vjepa_moe.modules import SoftMoEMLP
...
def main(args, resume_preempt=False):
    ...
    # -------------------------------------------------------------------------
    # [Dynamic MoE Injection]
    # -------------------------------------------------------------------------
    use_moe = cfgs_model.get("use_moe", False)
    if use_moe:
        moe_ratio = cfgs_model.get("moe_ratio", 0.25)
        num_experts = cfgs_model.get("num_experts", 4)
        logger.info(f"[MoE] Converting models to MoE (ratio={moe_ratio}, experts={num_experts})...")
        
        def convert_to_moe(model, device):
            ...
            if not hasattr(vit, 'blocks'):
                logger.warning("[MoE] Model does not have 'blocks', skipping conversion")
                return

            depth = len(vit.blocks)
            moe_start_layer = int(depth * (1 - moe_ratio))
            
            for i in range(moe_start_layer, depth):
                block = vit.blocks[i]
                original_mlp = block.mlp
                
                if isinstance(original_mlp, SoftMoEMLP):
                    continue
                    
                dim = original_mlp.fc1.in_features
                
                # Determine hidden dimension
                if hasattr(original_mlp, 'fc1'):
                    hidden_dim = original_mlp.fc1.out_features
                else:
                    hidden_dim = int(dim * 4.0)

                # Determine activation and dropout
                if hasattr(original_mlp, 'act'):
                    act_layer = type(original_mlp.act)
                else:
                    act_layer = torch.nn.GELU
                
                drop = original_mlp.drop.p if hasattr(original_mlp, 'drop') else 0.0
                ...
                # Initialize Experts from original MLP weights
                is_swiglu = hasattr(original_mlp, 'fc3')
                
                for expert_idx in range(num_experts):
                    expert = moe_mlp.experts[expert_idx]
                    with torch.no_grad():
                        # Copy FC1
                        expert.fc1.weight.data.copy_(original_mlp.fc1.weight.data)
                        if original_mlp.fc1.bias is not None:
                            expert.fc1.bias.data.copy_(original_mlp.fc1.bias.data)
                        
                        # Copy FC2
                        expert.fc2.weight.data.copy_(original_mlp.fc2.weight.data)
                        if original_mlp.fc2.bias is not None:
                            expert.fc2.bias.data.copy_(original_mlp.fc2.bias.data)
                            
                        # Copy FC3 (if SwiGLU)
                        if is_swiglu and hasattr(expert, 'fc3') and hasattr(original_mlp, 'fc3'):
                            expert.fc3.weight.data.copy_(original_mlp.fc3.weight.data)
                            if original_mlp.fc3.bias is not None:
                                expert.fc3.bias.data.copy_(original_mlp.fc3.bias.data)
                        
                        # Add small noise to break symmetry
                        noise_std = 0.01 * original_mlp.fc1.weight.std()
                        expert.fc1.weight.data.add_(torch.randn_like(expert.fc1.weight) * noise_std)
                
                # Replace MLP in the block
                block.mlp = moe_mlp
        
        # Convert both encoder and target_encoder
        # Important: They must have the same architecture for EMA to work
        convert_to_moe(encoder, device)
        convert_to_moe(target_encoder, device)
        
        logger.info("[MoE] Conversion complete.")
