...
# Common launcher for Cholec80 anticipation probing.
#
# Semantic contract shared with the Python pipeline:
#   - CSV columns `ant_reg_*` store raw anticipation targets on the benchmark scale
#   - raw target == 0.0: target already present
#   - 0.0 < raw target < anticipation_horizon: target is inside horizon
#   - raw target >= anticipation_horizon: target is outside horizon
#
# During training:
#   - regression head fits clipped normalized targets in [0, 1]
#   - classification head fits the 3 horizon states above
# During evaluation:
#   - predictions are converted back to the raw horizon scale for MAE reporting
...
TMP_PREFIX="${TMP_PREFIX:-nsjepa_cholec80_anticipation}"
...
echo
echo "${TASK_NAME} probing finished."
