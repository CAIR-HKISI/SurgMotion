...
from evals.utils.bootstrap import print_bootstrap_results
...
def evaluate_per_video(predictions_df, phases=None, use_bootstrap=False, n_bootstrap=1000, random_seed=None, head_id=None):
    """
    Evaluate overall (dataset-level) metrics with optional bootstrap uncertainty estimation.
    ...
    Returns:
        per_video: None (kept for backward compatibility)
        stats: Aggregated statistics (global metrics, with bootstrap uncertainty if requested)
    ...
    # Sort predictions by video and temporal index以保证相同视频内部顺序一致
    predictions_df = predictions_df.sort_values(['vid', 'data_idx'])

    # 保存每个视频的预测序列，后续 bootstrap 仍按视频采样但最终只计算整体指标
    video_level_sequences = [
        subdf[['label', 'prediction']].copy()
        for _, subdf in predictions_df.groupby('vid')
    ]
    ...
    def _compute_global_metrics(gt, pred):
        """基于整体样本计算分类指标。"""
    ...
    def _bootstrap_global_metrics(sequences, metric_keys):
        if len(sequences) == 0:
            raise ValueError("No video data available for bootstrap sampling.")

        rng = np.random.RandomState(random_seed)
        n_videos = len(sequences)
        bootstrap_records = {metric: [] for metric in metric_keys}

        for _ in range(n_bootstrap):
            indices = rng.choice(n_videos, size=n_videos, replace=True)
            sampled = pd.concat([sequences[idx] for idx in indices], ignore_index=True)
            sampled_metrics = _compute_global_metrics(
                sampled['label'].values,
                sampled['prediction'].values,
            )
            for metric in metric_keys:
                bootstrap_records[metric].append(sampled_metrics[metric])
        ...
        for metric in metric_keys:
            values = np.array(bootstrap_records[metric])
            results['mean'][metric] = np.mean(values)
            results['std'][metric] = np.std(values)
            results['ci_lower'][metric] = np.percentile(values, 2.5)
            results['ci_upper'][metric] = np.percentile(values, 97.5)
        return results

    overall_metrics = _compute_global_metrics(all_gt, all_pred)
    metrics = ["Accuracy", "Macro_Precision", "Macro_Recall", "Macro_IoU", "Macro_F1"]
    stats = {}

    if use_bootstrap:
        head_str = f" for head_{head_id}" if head_id is not None else ""
        logger.info(f"Performing bootstrap with {n_bootstrap} iterations{head_str} (global metrics)...")
        bootstrap_results = _bootstrap_global_metrics(video_level_sequences, metrics)
        for m in metrics:
        ...
        for m in metrics:
            stats[f"{m}_Mean"] = overall_metrics[m]
            stats[f"{m}_Std"] = 0.0

    # Add per-class metrics to stats（直接用整体样本统计）
...
def main(args_eval, resume_preempt=False):
    ...
    # 仅支持分类任务，若配置指定其他类型则直接报错
    if args_eval.get("task_type", "classification") != "classification":
        raise ValueError("surgical_action_probing 仅支持分类任务。")
    ...
    if head_to_dataset_map is not None:
        ...
        # Default: all heads use 第一个数据集的类别数
        classifiers = [
            AttentiveClassifier(
                embed_dim=encoder.embed_dim,
                num_heads=num_heads,
    ...
    if dist.is_initialized():
        ...
        logger.info("Wrapped classification heads with DistributedDataParallel")
    else:
        logger.info("Running classification in single-process mode (no DDP wrapping)")
    ...
    # ----------------------
    # 按类别频数计算 class weights，用于提升少数类表现
    # ----------------------
    class_weights = None
    # Determine if we need to calculate weights based on loss_type
    needs_weights = "weighted" in loss_type or "cb" in loss_type
    ...
    if needs_weights:
        if hasattr(train_loader.dataset, "class_counts"):
            ...
            if "cb" in loss_type:
                ...
                # ----------------------
                # Standard Inverse Frequency (Fallback)
                # ----------------------
        ...
        with torch.no_grad():
            for idx, coutputs in enumerate(outputs):
                ...
                if head_to_dataset_map is not None and dataset_indices is not None:
                    ...
                    # else: no samples for this head in this batch
                else:
