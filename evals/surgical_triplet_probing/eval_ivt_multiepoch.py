...
from evals.surgical_triplet_probing.models import init_module # probing
# from evals.surgical_triplet_probing.models_unfrozen import init_module #  fineturing
...
def main(args_eval, resume_preempt=False):
    ...
    # args_model = args_pretrain.get("pretrain_kwargs")  # 使用使用unisurge 
    args_model = args_pretrain
    ...
    # 训练循环
    # ----------------
    train_mAP = ivtmetrics.Recognition(100)
    train_mAP.reset_global()
    test_mAP = ivtmetrics.Recognition(100)
    test_mAP.reset_global()
    print("Metrics built ...")
    
    criterion = F.binary_cross_entropy_with_logits
    train_acc_meters = [AverageMeter() for _ in classifiers]
    test_acc_meters = [AverageMeter() for _ in classifiers]
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # instrument
        train_mAP_i = train_mAP.compute_AP('i', ignore_null=True)
        print("ins mean AP: ", train_mAP_i["mAP"])
        # verb
        train_mAP_v = train_mAP.compute_AP('v', ignore_null=True)
        print("ver mean AP: ", train_mAP_v["mAP"])
        # target
        train_mAP_t = train_mAP.compute_AP('t', ignore_null=True)
        print("tar mean AP: ", train_mAP_t["mAP"])
        # instrument-verb association
        train_mAP_iv = train_mAP.compute_AP('iv', ignore_null=True)
        print("i-v mean AP: ", train_mAP_iv["mAP"])
        # instrument-target association
        train_mAP_it = train_mAP.compute_AP('it', ignore_null=True)
        print("i-t mean AP: ", train_mAP_it["mAP"])
        # triplet
        train_mAP_ivt = train_mAP.compute_AP('ivt', ignore_null=True)
        print("tri mean AP: ", train_mAP_ivt["mAP"])

        val_metrics = run_one_epoch(
            mAP=test_mAP,
            criterion=criterion,
            acc_meters=test_acc_meters,
            loss_meters=loss_meters,
            device=device,
            training=False,
            encoder=encoder,
        ...
        # instrument
        test_mAP_i = test_mAP.compute_AP('i', ignore_null=True)
        print("ins mean AP: ", test_mAP_i["mAP"])
        # verb
        test_mAP_v = test_mAP.compute_AP('v', ignore_null=True)
        print("ver mean AP: ", test_mAP_v["mAP"])
        # target
        test_mAP_t = test_mAP.compute_AP('t', ignore_null=True)
        print("tar mean AP: ", test_mAP_t["mAP"])
        # instrument-verb association
        test_mAP_iv = test_mAP.compute_AP('iv', ignore_null=True)
        print("i-v mean AP: ", test_mAP_iv["mAP"])
        # instrument-target association
        test_mAP_it = test_mAP.compute_AP('it', ignore_null=True)
        print("i-t mean AP: ", test_mAP_it["mAP"])
        # triplet
        test_mAP_ivt = test_mAP.compute_AP('ivt', ignore_null=True)
        print("tri mean AP: ", test_mAP_ivt["mAP"])
...
# ----------------------
def run_one_epoch(
    mAP, criterion, acc_meters, loss_meters,
    ...
    for itr, data in enumerate(data_loader): # ~ data sructure
        ...
        with torch.no_grad():
            for idx, coutputs in enumerate(outputs):
                tri_preds = torch.stack([F.sigmoid(o) for o in coutputs]).mean(0) # torch.Size([4, 6])
    ...
    if not training:
        ### save .pkl ###
        pkl_name = 'epoch' + str(epoch) + '_bs_' + str(batch_size) + '_test.pkl'
        file_name = os.path.join(folder, pkl_name)
...
def make_dataloader(
    ...
    transform = make_transforms(
        ...
        random_horizontal_flip=False, # True,
        random_resize_aspect_ratio=(0.9, 1.1), # (0.75, 4 / 3),
        random_resize_scale=(0.7, 1.0), # (0.08, 1.0),
        reprob=0.25, # 0.25,
        auto_augment=True, # True,
        motion_shift=False,
        crop_size=img_size,
