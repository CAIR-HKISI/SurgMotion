...
# print('check_path', os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
...
class SurgicalVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if dataset_fpcs is None:
            self.dataset_fpcs = [frames_per_clip for _ in data_paths] # frames_per_clip = 16; self.dataset_fpcs = [16, 16, ...]
        else:
        ...
        for data_path in self.data_paths:
            if data_path.endswith(".csv"):
                try:
                    ...
                    required_columns = ['clip_path', 'label_path', 'case_id', "Index"] #, "pha"]
                    # clip_path: data/Surge_Frames/Cholec80/clips_16f/clip_dense_16f_info/test/case41_c000_f-00015-000001_padded.txt (.txt_path)
                    # label:     0 (phase_label) -> "[0, 2]","[0, 9]","[0, 14]","[7, 96]","[0, 0]" ("ins", "ver", "tar", "tri", "pha")
                    # case_id:   41 (VID41)
                    # Index:     idx of .txt files
                    ...
                    for _, row in data.iterrows():
                        all_label = np.loadtxt(row['label_path'], dtype=np.int32, delimiter=',')

                        label_combined = [[all_label, hash(str(row['case_id'])) % (2**31), int(row['Index'])]]
                        labels += label_combined
                    # -------------------------------------------------------------------------------
                    ...
                    # -------------------------------------------------------------------------------
                    # 样本 1 的标签: [1, 2063900493, 1, 0] (格式: [label, case_id, idx])
                    # 标签类型: <class 'list'>, 元素类型: <class 'int'>, <class 'int'>
                    # 样本 2 的标签: [1, 2063900493, 2, 0] (格式: [label, case_id, idx])
                    # 标签类型: <class 'list'>, 元素类型: <class 'int'>, <class 'int'>
                    # 样本 3 的标签: [1, 2063900493, 3, 0] (格式: [label, case_id, idx])
                    # 标签类型: <class 'list'>, 元素类型: <class 'int'>, <class 'int'>
        ...
        while not loaded_sample:
            if not isinstance(sample, str):
                ...
                if sample.split(".")[-1].lower() in ("txt"): # ~ 
                    loaded_sample = self.get_item_frames(index) # ~ return buffer, label_with_dataset, clip_indices
    ...
    def get_item_frames(self, index): # ~
        ...
        buffer, clip_indices = self.load_frames_from_txt(sample, frames_per_clip) # ~ return np.array(buffer), clip_indices
        if len(buffer) == 0:
            return None  # 加载失败时返回None
        ### label ### instrument 6, verb 10, target 15, triplet 100, phase 7
        label = self.labels[index] # [label, case_id]
        ...
        return buffer, label_with_dataset, clip_indices # 返回列表形式的clip
    ...
    def load_frames_from_txt(self, txt_path, fpc): # ~
        ...
        try:
            with open(txt_path, 'r') as f:
                ...
                # frame_paths = [line.split(',')[0] for line in f]
    ...
    def to_binary_anno(self, num, length):
        one_hot_encoding = np.zeros(length+1, dtype=np.int32)
        # set 1 at corresponding position.
        one_hot_encoding[num] = 1
        return one_hot_encoding[:-1]
...
if __name__ == "__main__":
    ...
    cholect50 = "/home/yaxin_hu/projects/NSJepa-triplet_recognition/data/CholecT50/clips_16f/try_dense_16f_detailed.csv"
    ...
    data = SurgicalVideoDataset(
        ...
        data_paths=[cholect50],
        datasets_weights=[0.04],
        frames_per_clip=16,
        fps=None,
        dataset_fpcs=None,
    ...
    for i in range(1, len(data)):
        item = data[i]
        buffer, labels, clip_indices = item # clips: item[0]; labels: item[1][0]; dataset_idx: item[1][3] 
        print(f"样本 {i} 的标签: {len(labels)} (格式: [label, case_id, idx])")
        print(f"标签类型: {type(labels)}, 元素类型: {type(labels[0])}, {type(labels[1])}")
