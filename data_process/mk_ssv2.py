...
# 读取 /data/wjl/vjepa2/data/ssv2/labels/train.json，制作成csv
# 要求，格式如下
# /absolute_file_path.[mp4, webm, etc.] $integer_class_label
...
def process_train_json(train_json_path, label_map, video_root, output_csv_path):
    """
    处理train.json，生成csv
    """
    ...
    for item in data:
        ...
        if template not in label_map:
            print(f"未找到标签: {template}")
            continue
        label = label_map[template]
        # 构造视频路径
        # 假设视频文件为webm格式
        video_path = os.path.join(video_root, f"{vid}.webm")
        if not os.path.exists(video_path):
            # 也可能是mp4等格式
            found = False
            for ext in ['mp4', 'webm', 'avi', 'mov']:
                video_path_try = os.path.join(video_root, f"{vid}.{ext}")
                if os.path.exists(video_path_try):
                    video_path = video_path_try
                    found = True
                    break
            if not found:
                print(f"未找到视频文件: {vid}")
...
if __name__ == "__main__":
    # 路径配置
    train_json_path = "/data/wjl/vjepa2/data/ssv2/labels/train.json"
    label_json_path = "data/ssv2/labels/labels.json"
    video_root = "/data/wjl/vjepa2/data/ssv2/20bn-something-something-v2"
    output_csv_path = "train_ssv2.csv"

    label_map = load_label_map(label_json_path)
    process_train_json(train_json_path, label_map, video_root, output_csv_path)
