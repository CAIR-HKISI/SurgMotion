...
_mamba_ssm_dir = os.path.dirname(_current_dir)  # mamba_ssm 目录
if _mamba_ssm_dir not in sys.path:
    sys.path.insert(0, _mamba_ssm_dir)
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, T=1, return_last_state=None):
        """
        hidden_states: (B, L, D)
        Returns: same shape as hidden_states
        """
        return_last_state = self.return_last_state if return_last_state is None else return_last_state

        batch, seqlen, dim = hidden_states.shape

        if self.bimamba:
            conv_state, ssm_state, conv_state_b, ssm_state_b = self._get_states_from_cache(inference_params, batch)
        else:
        ...
        if self.use_fast_path:  # Doesn't support outputting the states
            if self.bimamba:
                ...
                out_b = mamba_inner_fn_no_out_proj(
                    ...
                    delta_softplus=True,
                    ssm_state=ssm_state_b,
                    conv_state=conv_state_b,
                    return_last_state=return_last_state
                )
        ...
        if inference_params is not None:
            ...
            if self.bimamba:
                inference_params.key_value_memory_dict[self.layer_idx] = (conv_state, ssm_state, conv_state_b, ssm_state_b)
            else:
                inference_params.key_value_memory_dict[self.layer_idx] = (conv_state, ssm_state)
            return out, inference_params
        else:
            return out
    ...
    def _get_states_from_cache(self, inference_params, batch_size, initialize_states=False):
            ...
            if inference_params is None or self.layer_idx not in inference_params.key_value_memory_dict:
                ...
                if self.bimamba:
                    conv_state, ssm_state, conv_state_b, ssm_state_b = inference_params.key_value_memory_dict[self.layer_idx]
                else:
                ...
                if initialize_states:
                    ...
                    if self.bimamba:
                        ...
                        ssm_state_b.zero_()
            if self.bimamba:
                return conv_state, ssm_state, conv_state_b, ssm_state_b
            else:
                return conv_state, ssm_state
