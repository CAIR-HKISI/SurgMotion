    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., use_flash_attn=False,
        ...
        if use_flash_attn:
            if FlashAttention is None:
                # Fallback if somehow use_flash_attn is True but module missing
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if use_fused_mlp and FusedMLP is not None:
            ...
            if use_fused_mlp:
                # Fallback warnings handled at higher level usually, but just in case
                pass
...
class InternVL_CLIP(nn.Module):
    def __init__(
        ...
        if use_flash_attn and FlashAttention is None:
            print("Warning: FlashAttention not found, forcing use_flash_attn=False")
            use_flash_attn = False
            
        if use_fused_rmsnorm and DropoutAddRMSNorm is None:
            print("Warning: DropoutAddRMSNorm not found, forcing use_fused_rmsnorm=False")
            use_fused_rmsnorm = False
            
        if use_fused_mlp and FusedMLP is None:
            print("Warning: FusedMLP not found, forcing use_fused_mlp=False")
            use_fused_mlp = False
