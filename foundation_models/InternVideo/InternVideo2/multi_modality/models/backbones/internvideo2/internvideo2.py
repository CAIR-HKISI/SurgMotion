...
try:
    ...
    try:
        from foundation_models.InternVideo.InternVideo2.multi_modality.models.backbones.internvideo2.flash_attention_class import FlashAttention
    except ImportError:
        # Fallback if we cannot import flash attention class
        import sys
        import os
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if use_fused_mlp and FusedMLP is not None:
...
class PretrainInternVideo2(nn.Module):
    def __init__(
        ...
        if use_flash_attn and FlashAttention is None:
            logger.warn("FlashAttention not found, forcing use_flash_attn=False")
            use_flash_attn = False
            
        if use_fused_rmsnorm and DropoutAddRMSNorm is None:
            logger.warn("DropoutAddRMSNorm not found, forcing use_fused_rmsnorm=False")
            use_fused_rmsnorm = False
            
        if use_fused_mlp and FusedMLP is None:
            logger.warn("FusedMLP not found, forcing use_fused_mlp=False")
            use_fused_mlp = False
